...
@section('content')
    <section class="section">
        <div class="section-body">
                            ...
                            <table class="table table-hover" id="table">
                                ...
                                <tbody>
                                    @foreach ($barang as $item)
                                        <tr>
                                            <form action="{{ url(auth()->user()->level . '/penjualan/store') }}"
                                                method="POST">
                                                @csrf
            ...
            </div><hr style="border-top: 2px solid black; font-weight: bold;">
                                    ...
                                    <tbody>
                                        @foreach ($transaksi_sementara as $item)
                                            <tr>
                                                ...
                                                <form
                                                    action="{{ url('/kasir/transaksi-sementara/' . $item->id . '/' . $item->barang_id . '/edit') }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    ...
                                                    <td class="diskon" value="{{ $item->diskon }}">
                                                        {{ $item->diskon }}% <input type="text"
                                                            value="{{ $item->diskon }}" name="diskon" hidden></td>
                                                ...
                                                <form
                                                    action="{{ url('/' . auth()->user()->level . '/penjualan/' . $item->id) }}"
                                                    id="delete-form">
                                                    @csrf
            ...
            <div class="card-footer">
                ...
                <a href="{{ url('/' . auth()->user()->level . '/penjualan/hapus/semua') }}"
